...
var classScheduleListingExtractor = function ($) { return __awaiter(void 0, void 0, void 0, function () {
    var regex, classSchedules, sectionEntries, sectionIdx, classSchedule, title, m, titleRegex, titleInfo, sectionEntry, scheduleEntries, sectionInfo, additionalNotesRegex, associatedTermRegex, associatedStartRegex, associatedEndRegex, registrationDatesRegex, registrationStartRegex, registrationEndRegex, yearRegex, levelsRegex, campusRegex, sectionTypeRegex, instructionalMethodRegex, creditsRegex, associatedTerm, associatedStart, associatedEnd, year, registrationDates, registrationStart, registrationEnd, levels, meetingTimes;
    return __generator(this, function (_a) {
        (0, assertions_1.assertPageTitle)('Class Schedule Listing', $);
        ...
        for (sectionIdx = 0; sectionIdx < sectionEntries.length; sectionIdx += 2) {
            ...
            titleRegex = /(.*) - \d{5}/;
            titleInfo = titleRegex.exec(title.text())[1].trim();
            ...
            sectionInfo = $("tr td", sectionEntry)
                ...
                .text()
                .trim();
            additionalNotesRegex = /(.*)Associated Term/s;
            ...
            if (associatedTermRegex.test(sectionInfo)) {
                ...
                if (associatedEndRegex.test(associatedTerm) &&
                    ...
                    associatedStart = (0, dayjs_1.default)(associatedStartRegex.exec(associatedTerm)[1], 'MMM').format('MM');
                    associatedEnd = (0, dayjs_1.default)(associatedEndRegex.exec(associatedTerm)[1], 'MMM').format('MM');
            ...
            if (levelsRegex.test(sectionInfo)) {
                levels = levelsRegex.exec(sectionInfo)[1].toLowerCase().trim();
                classSchedule.levels = levels.split(/,\s*/);
            }
            ...
            if (sectionTypeRegex.test(sectionInfo)) {
                classSchedule.sectionType = sectionTypeRegex.exec(sectionInfo)[1].toLowerCase();
            }
            ...
            while (true) {
                ...
                meetingTimes.push({
                    ...
                    instructors: scheduleEntries[6].split(/\s*,\s*/).map(function (instructor) { return instructor.replace(/\s+/g, ' ').trim(); }),
                });
            }
            classSchedule.meetingTimes = meetingTimes;
            classSchedules.push(classSchedule);
        }
        return [2 /*return*/, classSchedules];
    });
}); };
exports.classScheduleListingExtractor = classScheduleListingExtractor;
